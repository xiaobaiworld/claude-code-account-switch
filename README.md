...
**用量监控守护进程**（v3.8.7+，v3.8.8/v3.8.9 演进）：

独立轮询用量、在撞墙前主动切换，不再依赖"说了话 Claude 答完才检测"。两条 spawn 入口：

1. **状态栏路径**：active 5h ≥ 90% 时由状态栏 tick spawn（节省 API 调用）
2. **Web UI 路径**（v3.8.8+）：在「账号用量监控」区块勾选启用，无视当前用量直接 spawn，**不爱用状态栏的人也能用**

轮询频率：

- < 90%：每 60s 查一次（仅 Web UI 路径会到这里，状态栏路径根本不会启动）
- 90–95%：每 60s
- 96–98%：每 10s
- ≥ 99%：立即切换，切换成功才退出（v3.8.9 修复：之前会切换失败也退出，全满场景无法等到候选自然 reset）
- API 返回 429（active 撞墙信号）：force 切换后退出

退出条件（v3.8.9 收紧）：

- 切换成功
- `~/.ccs/usage-monitor.disabled` 文件存在
- 运行超过 7 天（实质无限，防意外泄漏）
- ~~低于 90% 自动退出~~（v3.8.9 移除，否则 Web UI 启用后会立即退出）
- ~~连续 5 次错误退出~~（v3.8.9 改为指数退避 60/120/240/300s 持续重试，网络抖动自愈）

单例保护：`~/.ccs/usage-monitor.pid` + 进程探活（Windows 走 `OpenProcess`+`GetExitCodeProcess`，Unix 走 `os.kill(pid,0)`）+ `atexit` 清理。

在 Web UI 的「Claude Code 状态栏」区块点「安装」即可装好状态栏：同时复制 `statusline-command.sh`、`auto_switch_core.py`、`usage_monitor.py` 到 `~/.claude/`，并写入 `~/.claude/settings.json` 的 `statusLine` 字段。卸载同理一键完成。
「账号用量监控」区块开关与状态栏独立——不装状态栏也能单独启用守护。
