...
**架构：1 中央节点（hub） + N 个边缘节点（spoke）**

- **中央节点**：HTTP 服务端，不主动发请求，只响应所有边缘的查询。**它是数据的权威源**（所有边缘都跟它对账）
- **边缘节点**：每 30 秒访问中央，按账号粒度比较 hash + updatedAt：自己新就 push 给中央，中央新就从中央 pull。任意边缘的改动都会下一次轮询时同步到中央，然后通过其他边缘的轮询扩散到所有节点

`activeAccount` 和账号删除不同步，各端独立。

> 术语对应代码：「中央节点」= 配置里 peerUrl 留空（之前文档里叫"被动方"）。「边缘节点」= 配置 peerUrl 指向中央（之前叫"主动方"）。

### Web UI 配置（推荐）

**Step 1 — 起中央节点**（任选一台稳定常驻的机器，建议绑 0.0.0.0 让 LAN 可达）：
...
角色   : 被动方（等待对端访问）   ← 这就是中央节点
```

记下 URL 和 Secret。

**Step 2 — 每个边缘节点都填同一个中央 URL + Secret**：

打开 Web UI「多端共享同步」区域：

- 勾选启用
- **对端 URL** = 中央节点的 URL（Step 1 那个）
- **共享密钥** = 中央节点的 Secret
- 保存
...
N 个边缘节点重复 Step 2 即可，**它们之间不需要互联**，全靠中央扇出。
